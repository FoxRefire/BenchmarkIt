// Shared benchmark fetch + parse logic (used by service worker, offscreen, or Firefox background page)
...
async function findGpuIdFromGPUList(text) {
    ...
    if (!foundGpu) {
        throw new Error("GPU not found in PassMark list");
    }
...
export async function getResult(text, productType) {
    text = normalizeText(text);
    if (productType === "cpu") {
        const result = await getCPUPassmark(text);
        return { result };
    }
    if (productType === "gpu") {
        const result = await getGPUPassmark(text);
        return { result };
    }
    if (productType === "mobile") {
        const result = await getMobileNanoReview(text);
        return { result };
    }
    return { result: null };
}
